...
#Acá especifica las máquinas a crear:

#---DB--- Selecciono un volúmen que se utilizará en DB, luego crea db:
...
#@test -n "$(VOL_ID)" || { echo 'ERROR: no hay volumen: "$(ENV)-db-vol"'; exit 1;}
	$(eval VOL_ID:=$(shell openstack volume list -f value |awk '/relgueta_vol01/ {print $$1 }'))
	@test -n "$(VOL_ID)" || { echo 'EROOR: no hay volumen: "relgueta_vol01"'; exit 1;}
	make do-deploy-db  EXTRA="--block-device-mapping vdb=$(VOL_ID):::0"

#---WEB--- Creo web y traigo la variable con la cantidad de webs a generar:

deploy-web:
	make do-deploy-web EXTRA="--min-count $(CantWeb)"
...
#USUARIO DE LINUX: trae el usuario de Launch Pad (Ubuntu one) - a partir del usuario actual activo que invoca - ademas toma al usr activo para generar los nombres de las maquinas

LP_USER:=$(shell sed -n 1s/.*ssh-import-id.//p  ~/.ssh/authorized_keys)

%.tmp.yaml: %.yaml
	@echo "EDITAR_USUARIO=$(USER) EDITAR_LP=$(LP_USER)"
	@sed -e "s/EDITAR_USUARIO/$(USER)/" -e "s/EDITAR_LP/$(LP_USER)/" -e "s/EDITAR_ENV/$(ENV)/" -e "s/NRO_WEBS/$(CantWeb)/" $^ > $@
